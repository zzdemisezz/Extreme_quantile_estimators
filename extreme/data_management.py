...
class DataSampler():
    ...
    def load_simulated_data(self, n_data, rep=0, saved=False):
        ...
        if pathfile_data.is_file() and pathfile_Xorder.is_file():  # if file exists, load existing data
            ...
            #so this is what happens in data folder I guess
...
#Datasampler for the real dataset
class DataSampler_real():
    def __init__(self, distribution, percentile=0, **kwargs):
        self.distribution = distribution

        self.ht_dist = pd.read_csv("dataset/be_firelosses3.csv").loc[:, 'Claims'].to_numpy()
    ...
    def load_simulated_data(self, n_data, rep=0, saved=False):
        ...
        data_filename = "xyz_ndata{}-rep{}.npz".format(self.distribution, n_data, rep)
        Xorder_filename = "Xorder_ndata{}-rep{}.npy".format(self.distribution, n_data, rep)
    ...
    #Only used to return order statistics, hence parameter 'random' is unnecessary
    def simulate_quantiles(self, n_data, low_bound=0., high_bound=1., random=True, seed=32, **kwargs):
        """
        simulate from quantile function  q
        quantiles(random=False) or order statistics (random=True) from heavy-tailed distribution
        ...
        up_bound :
        random : bool
            if true: return order statistics
        kwargs :
        ...
        if random:
            np.random.seed(seed)
            return np.sort(self.ht_dist)  # sort the order statistics
        else:
            u_values = np.linspace(low_bound, high_bound, int(n_data)).reshape(-1,1)  # endpoint=False
            return np.float32(self.ht_dist.ppf(u_values))
