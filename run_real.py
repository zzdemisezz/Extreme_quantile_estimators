...
from extreme.data_management import DataSampler, DataSampler_real
...
data_sampler = DataSampler_real("be_firelosses", pams=None, percentile=0)
print(np.shape(data_sampler.ht_dist)[0])
n_data = 1323
...
def train_a_replication(replication=1):
    ...
    # INSURANCE DATA
    # pathfile = Path("ckpt", "real", "training", "{}.pt".format(model_filename_rep))
    # EARTHQUAKE DATA
    pathfile = Path("ckpt", "earthquakes", "training", "{}.pt".format(model_filename_rep))
    ...
    # regular training
    data, X_order = data_sampler.load_simulated_data(n_data=n_data, rep=replication) #don't really understand how this works
    ...
    #not sure if distribution is important here, nvm only important here for putting it in the right file
